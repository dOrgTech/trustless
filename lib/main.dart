...
persist() async {
  ...
  actions = [];
  projectsCollection =
      FirebaseFirestore.instance.collection("projects${Human().chain.name}");
  transactionsCollection = FirebaseFirestore.instance
      .collection("transactions${Human().chain.name}");
  usersCollection =
      FirebaseFirestore.instance.collection("users${Human().chain.name}");
    ...
    return MaterialApp(
        ...
        onGenerateRoute: (settings) {
          ...
          if (settings.name == '/') {
            ...
            try {
              project = projects
                  .firstWhere((proj) => proj.contractAddress == projectId);
            } catch (e) {
              project = null;
            }
            if (project != null) {
            ...
            builder = (_) =>
                BaseScaffold(selectedItem: 3, body: Users(), title: "Users");
          } else if (settings.name == '/') {
            builder = (_) => BaseScaffold(
                selectedItem: 0, body: Landing(), title: "Trustless Business");
            ...
            builder = (_) => BaseScaffold(
                selectedItem: 1, body: AllActivity(), title: "Activity");
          } else {
...
class _BaseScaffoldState extends State<BaseScaffold> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget logo = Image.network(
      'https://i.ibb.co/QnyXWBP/bizlogo.png',
      height: widget.isTrustless ? 33 : 32,
    );
    Widget logotall = Image.network(
      'https://i.ibb.co/QnyXWBP/bizlogo.png',
      height: 72,
    );
    ...
    final TextStyle selectedMenuItem =
        TextStyle(fontSize: 19, color: blendedColor);
    final TextStyle nonSelectedMenuItem =
        TextStyle(fontSize: 16, color: textThemeColor);
                ...
                child: Row(
                  children: [
                    Icon(Icons.local_activity,
                        size: 30,
                        color: widget.isProjects
                            ? Theme.of(context).indicatorColor
                            : Theme.of(context).textTheme.bodyLarge!.color!),
                    const SizedBox(width: 8),
                    Text("PROJECTS",
                        style: widget.isProjects
                            ? selectedMenuItem
                            : nonSelectedMenuItem)
                  ],
                ),
                ...
                child: Row(
                  children: [
                    Icon(Icons.people_alt_outlined,
                        size: 33,
                        color: widget.isUsers
                            ? Theme.of(context).indicatorColor
                            : Theme.of(context).textTheme.bodyLarge!.color!),
                    const SizedBox(width: 8),
                    Text("USERS",
                        style: widget.isUsers
                            ? selectedMenuItem
                            : nonSelectedMenuItem)
                  ],
                ),
    ...
    return Scaffold(
      ...
      floatingActionButton: MediaQuery.of(context).size.aspectRatio > 1
      ...
      appBar: AppBar(
        ...
        title: MediaQuery.of(context).size.aspectRatio >= switchAspect
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: GoogleFonts.changa(
                        color: Theme.of(context).indicatorColor, fontSize: 19),
                  )
                ],
              )),
      ...
      body: ListView(
        children: [
          ...
          human.wrongChain ? const WrongChain() : widget.body,
        ],
      ),
      drawer: MediaQuery.of(context).size.aspectRatio <= switchAspect
          ? Drawer(
              child: Column(
                children: <Widget>[
                  SizedBox(height: 20),
...
const Color _lightThemeHighlightColor =
    Color.fromRGBO(124, 112, 93, 1); // For light theme
const Color _darkThemeHighlightColor =
    Color.fromARGB(255, 212, 195, 140); // For dark theme
...
final ThemeData lightTheme = ThemeData(
  ...
  primaryColor:
      const Color.fromARGB(255, 155, 155, 155), // Main background color
  ...
  textButtonTheme: TextButtonThemeData(
    style: TextButton.styleFrom(
      primary: const Color.fromARGB(255, 20, 20,
          20), // Ensuring text color is black and not the highlight color
    ),
  ),
...
final ThemeData darkTheme = ThemeData(
  ...
  textButtonTheme: TextButtonThemeData(
    style: TextButton.styleFrom(
      primary: Colors
          .white, // Ensuring text color is white and not the highlight color
    ),
  ),
  appBarTheme: const AppBarTheme(
